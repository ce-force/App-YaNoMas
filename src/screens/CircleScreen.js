    ...
    return (
      <View style={styles.item}>
        ...
        >
          ...
          <Button
            style={styles.addFriendButton}
            onPress={() => acceptFriendRequest({ title })}
          >
            Aceptar solicitud
          </Button>
          <Button
            style={styles.addFriendButton}
            onPress={() => denyFriendRequest({ title })}
          >
            Denegar solicitud
          </Button>
        </View>
    ...
    return (
      <View style={styles.item}>
        ...
        >
          ...
          >
            <Text style={styles.title}>{title}</Text>
            <Switch onValueChange={toggleSwitch} value={switchValues[0]} />
            <Button
              style={styles.addFriendButton}
              onPress={() => deleteFriend({ title })}
            >
              Eliminar
            </Button>
          </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={USERS}
        renderItem={({ item }) => (
          <ItemRequests title={(item.title, item.title)} />
        )}
      ...
      <FlatList
        data={USERS}
        renderItem={({ item }) => (
          <ItemFriend title={(item.title, item.title)} />
        )}
        keyExtractor={(item) => item.id}
      />
    </View>
